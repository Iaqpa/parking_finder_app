...
class nnAPI:
    ...
    def make_prediction(self, img_cv2, threshold=0.5, rect_th=1, text_size=3, text_th=3):
        # print(img_cv2)
        ...
        statement = text("""SELECT * FROM rectangles where camera_id={}""".format(6))
        ...
        for parking_area, overlap_areas in zip(np.array(parking_boxes), overlaps):
            ...
            if max_IoU_overlap < 0.15:
                ...
                cv2.rectangle(img_cv2, tuple([int(x1), int(y1)]), tuple([int(x2), int(y2)]), (0, 255, 0), 3)
                # Отмечаем, что мы нашли как минимум оно свободное место.
                free_space = True
            else:
                # Место всё ещё занято — рисуем красную рамку.
                cv2.rectangle(img_cv2, tuple([int(x1), int(y1)]), tuple([int(x2), int(y2)]), (0, 0, 255), 1)
